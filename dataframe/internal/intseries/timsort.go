// Basically a copy of https://github.com/psilva261/timsort/blob/master/timsort.go
// Licence available in TIMSORT-LICENCE
 ...
 */
func newTimSortI(a []int, index SortIndex) (self *timSortHandlerI) {
...
func TimSort(a SortIndex) (err error) {
...
func Ints(a []int, index SortIndex) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
	...
	if nRemaining < _MIN_MERGE {
		initRunLen, err := countRunAndMakeAscendingI(a, lo, hi, index)
		if err != nil {
			return err
		}

		return binarySortI(a, lo, hi, lo+initRunLen, index)
	}
	...
	for {
		// Identify next run
		runLen, err := countRunAndMakeAscendingI(a, lo, hi, index)
		if err != nil {
			return err
		}
		...
		if runLen < minRun {
			...
			if err = binarySortI(a, lo, lo+force, lo+runLen, index); err != nil {
				return err
			}
			runLen = force
		}
 ...
 */
func binarySortI(a []int, lo, hi, start int, index SortIndex) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscendingI(a []int, lo, hi int, index SortIndex) (int, error) {
	...
	if index.Less(a[runHi], a[lo]) { // Descending
		runHi++

		for runHi < hi && index.Less(a[runHi], a[runHi-1]) {
			runHi++
		}
 ...
 */
func (self *timSortHandlerI) pushRun(runBase, runLen int) {
 ...
 * entry to the method.
 */
func (self *timSortHandlerI) mergeCollapse() (err error) {
 ...
 */
func (self *timSortHandlerI) mergeForceCollapse() (err error) {
	for self.stackSize > 1 {
		n := self.stackSize - 2
 ...
 */
func (self *timSortHandlerI) mergeAt(i int) (err error) {
	if self.stackSize < 2 {
	 ...
	 */
	k, err := gallopRightI(self.a[base2], self.a, base1, len1, 0, self.index)
	if err != nil {
		return err
	}
	if k < 0 {
	 ...
	 */
	len2, err = gallopLeftI(self.a[base1+len1-1], self.a, base2, len2, len2-1, self.index)
	if err != nil {
		return
	}
	if len2 < 0 {
 ...
 *    should follow it.
 */
func gallopLeftI(key int, a []int, base, len, hint int, index SortIndex) (int, error) {
	...
	if index.Less(a[base+hint], key) {
		// Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
		maxOfs := len - hint
		for ofs < maxOfs && index.Less(a[base+hint+ofs], key) {
			lastOfs = ofs
			ofs = (ofs << 1) + 1
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRightI(key int, a []int, base, len, hint int, index SortIndex) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
	...
	ofs := 1
	lastOfs := 0
	if index.Less(key, a[base+hint]) {
		// Gallop left until a[b+hint - ofs] <= key < a[b+hint - lastOfs]
		maxOfs := hint + 1
		for ofs < maxOfs && index.Less(key, a[base+hint-ofs]) {
			lastOfs = ofs
			ofs = (ofs << 1) + 1
			if ofs <= 0 { // int overflow
				ofs = maxOfs
 ...
 */
func (self *timSortHandlerI) mergeLo(base1, len1, base2, len2 int) (err error) {
...
outer:
	for {
		...
		for {
			...
			if self.index.Less(a[cursor2], tmp[cursor1]) {
				a[dest] = a[cursor2]
				dest++
				cursor2++
				count2++
		...
		for {
			...
			count1, err = gallopRightI(a[cursor2], tmp, cursor1, len1, 0, self.index)
			if err != nil {
				return
			}
			if count1 != 0 {
			...
			count2, err = gallopLeftI(tmp[cursor1], a, cursor2, len2, 0, self.index)
			if err != nil {
				return
			}
			if count2 != 0 {
 ...
 * @param len2  length of second run to be merged (must be > 0)
 */
func (self *timSortHandlerI) mergeHi(base1, len1, base2, len2 int) (err error) {
	if len1 <= 0 || len2 <= 0 || base1+len1 != base2 {
		return errors.New("len1 > 0 && len2 > 0 && base1 + len1 == base2;")
	}
...
outer:
	for {
		...
		for {
			...
			if self.index.Less(tmp[cursor2], a[cursor1]) {
				a[dest] = a[cursor1]
				dest--
				cursor1--
				count1++
		...
		for {
			...
			if gr, err := gallopRightI(tmp[cursor2], a, base1, len1, len1-1, self.index); err == nil {
				count1 = len1 - gr
			} else {
				return err
			}
			...
			if gl, err := gallopLeftI(a[cursor1], tmp, 0, len2, len2-1, self.index); err == nil {
				count2 = len2 - gl
			} else {
				return err
			}
 ...
 */
func (self *timSortHandlerI) ensureCapacity(minCapacity int) []int {
