...
//
// Time complexity O(m * n) where m = number of columns in qframe, n = number of rows.
...
// Drop creates a new projection of the QFrame without the specified columns.
//
// Time complexity O(1).
...
// ReadQBin reads a stream of QBin bytes and turns it into a QFrame. For more information
// on QBin see ToQBin.
func ReadQBin(reader io.Reader) QFrame {
	qbf, err := qfio.ReadQBin(reader)
	if err != nil {
		return QFrame{Err: err}

	}

	namedColumns := make([]namedColumn, len(qbf.Columns))
	columnsByName := make(map[string]namedColumn, len(qbf.Columns))
	for i, qbColumn := range qbf.Columns {
		col := namedColumn{
			Column: qbColumn.Column,
			name:   qbColumn.Name,
			pos:    i,
		}
		namedColumns[i] = col
	...
	return QFrame{columns: namedColumns, columnsByName: columnsByName, index: qbf.Index, Err: nil}
}
...
// ToQBin writes a QFrame to a stream of bytes in QBin format.
//
// QBin is a serialized representation of the QFrame optimized for speed of writing and reading.
//
// QBin is not meant to be a general purpose serialization format that is stable over space and time.
// What this means is that:
//   - There are no guarantees that the generated bytes will be the same or interoperable across different
//     computer architectures.
//   - There are no guarantees that the generated bytes will be the same or interoperable across different
//     versions of QFrame or Go.
func (qf QFrame) ToQBin(w io.Writer) error {
	if qf.Err != nil {
		return fmt.Errorf("error writing qbin, cannot write frame with error: %w", qf.Err)
	}
